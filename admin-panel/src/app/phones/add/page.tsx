...
export default function AddPhone() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrl = "";
      if (imageFile) {
        imageUrl = await uploadProductImage(imageFile, "phones");
      }
      ...
      alert("Failed to add phone. See console for details.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/phones" className="p-2 rounded-xl border border-border hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h2 className="text-2xl font-heading font-bold">Add New Phone</h2>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-1.5">Phone Name</label>
              <input
                type="text"
                required
                value={formData.name}
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">Storage</label>
              <input
                type="text"
                required
          ...
          <div className="pt-4 flex justify-end gap-4">
            ...
            <Button type="submit" disabled={loading} className="min-w-[120px]">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Save Phone"}
            </Button>
          </div>
        </form>
      </div>
