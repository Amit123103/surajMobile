...
export default function AddPhone() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrl = "";
      if (imageFile) {
        const storageRef = ref(storage, `repairs/${Date.now()}_${imageFile.name}`);
      ...
      alert("Failed to add repair. See console for details.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/repairs" className="p-2 rounded-xl border border-border hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h2 className="text-2xl font-heading font-bold">Add New Repair</h2>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 flex justify-end gap-4">
            ...
            <Button type="submit" disabled={loading} className="min-w-[120px]">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Save Repair"}
            </Button>
          </div>
        </form>
      </div>
