...
export default function AddPrinting() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error adding printjob:", error);
      alert("Failed to add printjob. See console for details.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/printing" className="p-2 rounded-xl border border-border hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h2 className="text-2xl font-heading font-bold">Add New PrintJob</h2>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-1.5">PrintJob ServiceName</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">PaperSize</label>
              <input
                type="text"
                required
          ...
          <div className="pt-4 flex justify-end gap-4">
            ...
            <Button type="submit" disabled={loading} className="min-w-[120px]">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Save PrintJob"}
            </Button>
          </div>
        </form>
      </div>
