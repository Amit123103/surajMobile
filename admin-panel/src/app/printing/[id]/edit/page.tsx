...
export default function EditPhone({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchPhone = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          alert("PrintJob not found!");
          router.push("/printing");
        }
      } catch (error) {
        console.error("Error fetching printjob:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to update printjob. See console for details.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-heading font-bold">Edit PrintJob</h2>
      </div>
